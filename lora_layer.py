...
class Linear(nn.Linear, LoraLayer):
    ...
    def merge(self):
        ...
        if self.r[self.active_adapter] > 0:
            ...
            # flag to True.
            self.weight.data += self.lora_B[self.active_adapter].weight @ self.lora_a[self.active_adapter].weight * self.scaling[self.active_adapter]
            self.merged = True
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.disable_adapters:
            ...
            # If the LoRA adapter is active and not merged, add the output of the LoRA layers to the result. This involves
            ...
            result += (self.lora_A[self.active_adapter](x) @ self.lora_B[self.active_adapter].weight.T) * self.scaling[self.active_adapter]
        else:
            result = F.linear(x, transpose(self.weight, self.fan_in_fan_out), bias=self.bias)
