...
class Trainer:
    ...
    def set_mixed_precision_context(self, mixed_precision_dtype):
        ...
        if mixed_precision_dtype is None:
            ...
            # Otherwise, use 'torch.amp.autocast' context with the specified dtype, and initialize GradScaler if mixed_precision_dtype is float16.
    ...
    def _run_batch(self, batch):
        ...
        # If 'mixed_precision_dtype' is torch.float16, you have to modify the backward using the gradscaler.
    ...
    def prepare_dataloader(self, train_dataset, eval_dataset):
        # Prepare the training DataLoader. Initialize 'DataLoader' with 'train_dataset' 
        ...
        data_trainloader = DataLoader(
            ...
            sampler=None if not self.is_ddp_training else DistributedSampler(train_dataset),
            collate_fn=DataCollatorForSeq2Seq(tokenizer=self.tokenizer, padding=True, return_tensors="pt")
        )

        # Prepare the evaluation DataLoader. Initialize 'DataLoader' with 'eval_dataset', 
